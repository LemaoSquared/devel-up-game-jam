...
func _ready() -> void:
	...
	$Start2.pivot_offset = $Start2.size / 2.0
	$Start2.mouse_entered.connect(_on_start2_hovered)
	$Start2.mouse_exited.connect(_on_start2_unhovered)
...
func _on_start2_hovered() -> void:
	...
	hover_tween2.tween_property($Start2, "rotation_degrees", 360.0, 1.5).as_relative().set_trans(Tween.TRANS_LINEAR)

func _on_start2_unhovered() -> void:
	if is_transitioning:
		return
	...
	$Start2.rotation_degrees = wrapf($Start2.rotation_degrees, 0.0, 360.0)
	var target_rotation = 360.0 if $Start2.rotation_degrees > 180.0 else 0.0
	
	hover_tween2 = create_tween()
	hover_tween2.tween_property($Start2, "rotation_degrees", target_rotation, 0.2).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	hover_tween2.tween_callback(func(): $Start2.rotation_degrees = 0.0)
...
func _on_start2_pressed() -> void:
	...
	var bounce_tween = create_tween()
	bounce_tween.tween_property($Start2, "position:y", $Start2.position.y - 20.0, 0.15).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	bounce_tween.tween_property($Start2, "position:y", $Start2.position.y, 0.3).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
	...
	var obj = OBJECT_SCENE.instantiate()
	get_tree().current_scene.add_child(obj)
	obj.global_position = $Start2.global_position
